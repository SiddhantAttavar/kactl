...
template<typename T, int SZ> struct SegTreeRecursive {
	vector<T> seg; T id; T (*cmb) (T, T);
	SegTreeRecursive(T _id, T _cmb(T, T)):id(_id),seg(2*SZ,id),cmb(_cmb){}
	...
	void upd(int i, T x, int s=0, int e=SZ-1, int c=1) {
		if (s > i || e < i) return;

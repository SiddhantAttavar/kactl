 ...
 * Author: Subham Pan
 * Date: 2025-12-21
 * Description: Graph initially empty. After adding each received edge, output
current number of bridges in the graph (stored in "bridges").
 * Time: O(NlogN + MlogN)
 */
#pragma once
...
int bridges; 
int lca_iteration; vector<int> last_visit;

void init(int n) {
    par.resize(n);
    dsu_2ecc.resize(n); dsu_cc.resize(n);
...
void merge_path (int a, int b) {
    ++lca_iteration;
    vector<int> path_a, path_b; int lca = -1;
    while (lca == -1) {
        if (a != -1) {
